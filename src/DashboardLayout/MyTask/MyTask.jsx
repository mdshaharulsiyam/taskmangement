    ...
    return (
        <div className='px-3'>
            {showtaskform && <>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className='w-72 md:w-[50%] flex flex-col items-start gap-2 justify-start z-40 absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]' >
            ...
            <div className='md:grid md:grid-cols-2 lg:grid-cols-3 gap-2 items-start justify-start '>
                ...
                <hr className='w-[100%] h-[2px] mx-auto bg-blue-500' />
                {
                    todoData.map(item=><div className={`relative px-2 overflow-hidden bg-yellow-50 shadow my-2 rounded`} key={item._id}>
                        ...
                        <span className='flex justify-start items-center p-2 gap-2'> <FaHourglassStart/> <FaCheck/> <FaRegEdit/> <MdDelete/> </span>
